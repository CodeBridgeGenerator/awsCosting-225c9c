...
const CostsEditDialogComponent = (props) => {
    ...
    const onSave = async () => {
        let _data = {
            date: _entity?.date,
linkedAccountId: _entity?.linkedAccountId,
linkedAccountName: _entity?.linkedAccountName,
service: _entity?.service,
usageType: _entity?.usageType,
itemDescription: _entity?.itemDescription,
resourceName: _entity?.resourceName,
resourceId: _entity?.resourceId,
availabilityZone: _entity?.availabilityZone,
unitPrice: _entity?.unitPrice,
usageQuantity: _entity?.usageQuantity,
amount: _entity?.amount,
previousMonthAmount: _entity?.previousMonthAmount,
monthlyGrowth: _entity?.monthlyGrowth,
tagDepartment: _entity?.tagDepartment,
tagApplication: _entity?.tagApplication,
tagEnvironment: _entity?.tagEnvironment,
tagOwner: _entity?.tagOwner,
tagProject: _entity?.tagProject,
        };

        setLoading(true);
        try {
            
        const result = await client.service("costs").patch(_entity._id, _data);
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info costs updated successfully" });
    ...
    return (
        <Dialog header="Edit Costs" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="costs-edit-dialog-component">
...
export default connect(mapState, mapDispatch)(CostsEditDialogComponent);
