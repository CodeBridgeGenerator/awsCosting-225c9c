...
import costsFakerFactory from "./costsFakerFactory";
...
const CostsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = costsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("costs").create(elem));
...
export default connect(mapState, mapDispatch)(CostsFakerDialogComponent);
